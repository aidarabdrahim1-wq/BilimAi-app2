...
const UBT_TOPICS: Record<string, { topics: string[], description: string }> = {
  "Қазақстан тарихы": {
    ...
    topics: ["ежелгі Қазақстан", "тас дәуірі, қола дәуірі, ерте темір дәуірі", "сақ, ғұн, үйсін, қаңлы", "түрік қағанаттары", "орта ғасыр мемлекеттері", "Қарахан, Қыпшақ, Найман, Керейіт, Жалайыр", "Алтын Орда, Ақ Орда, Моғолстан, Ноғай Ордасы, Әбілқайыр хандығы", "Қазақ хандығының құрылуы мен дамуы", "жоңғар шапқыншылығы", "Ресей империясы тұсындағы Қазақстан", "ұлт-азаттық көтерілістер", "ХХ ғасыр басы, Алаш қозғалысы", "кеңестік кезең, ашаршылық, қуғын-сүргін, ҰОС", "тәуелсіз Қазақстан, Конституциялар, саяси реформалар, экономика, мәдениет, халықаралық қатынастар"]
  },
  "Оқу сауаттылығы": {
    description: "Мәтінді талдау, интерпретациялау және логикалық қорытынды жасау дағдылары.",
    topics: ["мәтіннің тақырыбы мен негізгі ойы", "мәтіндегі ашық және жасырын ақпарат", "стиль түрлері", "мәтін құрылымы", "логикалық байланыс", "автор көзқарасы", "факті мен пікірді ажырату", "қорытынды шығару", "салыстыру", "мәтін бойынша интерпретация", "бірнеше мәтінді салыстырып талдау", "кесте, сызба, диаграммадағы ақпаратты оқу"]
  },
  "Математикалық сауаттылық": {
    description: "Күнделікті өмірдегі сандық мәліметтерді талдау және логикалық есептер.",
    topics: ["сан және есептеу", "бөлшек, пайыз, пропорция", "орташа мән", "қозғалыс, жұмыс, қоспа есептері", "кесте, график, диаграмма талдау", "ықтималдықтың қарапайым элементтері", "практикалық геометрия", "уақыт, қашықтық, масса, көлем бірліктері", "қаржылық сауаттылық элементтері", "күнделікті өмірдегі логикалық-сандық есептер"]
  },
  "Математика": {
    description: "Алгебра, тригонометрия және геометрияның тереңдетілген курсы.",
    topics: ["сандар мен өрнектер", "теңдеулер мен теңсіздіктер", "функциялар және графиктер", "дәреже, түбір, логарифм", "тригонометрия", "арифметикалық және геометриялық прогрессия", "туынды және оның қолданылуы", "алғашқы функция және интегралдың бастапқы түсініктері", "векторлар", "координаталар әдісі", "планиметрия", "стереометрия", "комбинаторика және ықтималдық"]
  },
  "Физика": {
    description: "Классикалық механикадан бастап атомдық физикаға дейінгі негізгі заңдар.",
    topics: ["кинематика", "динамика", "сақталу заңдары", "статика және гидростатика", "молекулалық физика", "термодинамика", "электростатика", "tұрақты ток заңдары", "магнит өрісі", "электромагниттік индукция", "тербелістер мен толқындар", "оптика", "атом және ядро физикасы", "радиация", "астрономияның негізгі элементтері"]
  },
  "Химия": {
    description: "Заттардың құрылысы, қасиеттері және химиялық реакциялар.",
    topics: ["атом құрылысы", "периодтық заң", "химиялық байланыс", "зат құрылысы", "химиялық реакциялар және олардың заңдылықтары", "тотығу-тотықсыздану", "ерітінділер", "электролиттік диссоциация", "бейорганикалық қосылыстардың негізгі кластары", "металдар мен бейметалдар", "органикалық химияның бастамалары", "көмірсутектер", "спирттер, альдегидтер, карбон қышқылдары, эфирлер", "аминдер, аминқышқылдар, ақуыздар", "есептер мен сапалық реакциялар"]
  },
  "Биология": {
    description: "Тірі ағзалардың құрылысы, генетика және адам анатомиясы.",
    topics: ["жасуша теориясы", "органоидтар", "зат алмасу", "фотосинтез және тыныс алу", "генетика және тұқымқуалаушылық", "селекция", "эволюция", "экология", "адам анатомиясы мен физиологиясы", "жүйке, қанайналым, тыныс алу, ас қорыту, зәр шығару, эндокриндік жүйелер", "ботаника", "зоология", "микроорганизмдер", "biотехнология элементтері"]
  },
  "География": {
  ...
  "Дүниежүзі тарихы": {
    ...
    topics: ["ежелгі өркениеттер", "антикалық дүние", "орта ғасырлар", "феодалдық қоғам", "ислам өркениеті", "Қайта өрлеу", "Реформация", "ұлы географиялық ашулар", "буржуазиялық революциялар", "индустрияландыру", "отаршылдық", "І және ІІ дүниежүзілік соғыс", "Версаль-Вашингтон жүйесі", "қырғи-қабақ соғыс", "деколонизация", "халықаралық қатынастар", "ХХ-ХХІ ғасырдағы жаһандық үрдістер"]
  },
  "Құқық негіздері": {
    description: "Мемлекет және құқық теориясы, ҚР заңнамасы.",
    topics: ["мемлекет және құқық теориясы", "Конституция", "адам және азамат құқықтары", "сайлау жүйесі", "мемлекеттік басқару", "әкімшілік құқық", "азаматтық құқық", "еңбек құқығы", "отбасы құқығы", "қылмыстық құқық", "сыбайлас жемқорлыққа қарсы мәдениет", "сот жүйесі", "құқықтық жауапкершілік", "халықаралық құқықтың негізгі ұғымдары"]
  },
  "Информатика": {
    description: "Ақпараттық технологиялар мен программалау негіздері.",
    topics: ["ақпарат және ақпараттық процестер", "компьютер архитектурасы", "операциялық жүйелер", "файлдар және деректер", "ақпаратты кодтау", "логика элементтері", "алгоритмдеу", "программалау негіздері", "деректер қоры", "электрондық кестелер", "желілер және интернет", "киберқауіпсіздік", "веб-технология негіздері", "модельдеу", "цифрлық сауаттылық"]
  },
  "Ағылшын тілі": {
    description: "Reading, Grammar, and Vocabulary development.",
    topics: ["reading comprehension", "vocabulary in context", "grammar: tenses, passive voice, reported speech", "conditionals, modal verbs, articles", "prepositions, relative clauses, word formation", "sentence transformation", "cloze test", "matching", "everyday and academic topics"]
  },
  "Қазақ тілі": {
    description: "Тілдік нормалар мен грамматикалық сауаттылық.",
    topics: ["фонетика", "орфоэпия", "орфография", "лексика және фразеология", "сөзжасам", "морфология", "сөз таптары", "сөйлем мүшелері", "жай сөйлем", "құрмалас сөйлем", "синтаксис", "тыныс белгілері", "мәтін түрлері", "стильдер", "тілдік норма", "грамматикалық талдау"]
  },
  "Қазақ әдебиеті": {
    description: "Әдеби бағыттар, жанрлар және шығармаларды талдау.",
    topics: ["әдеби бағыттар мен жанрлар", "әдеби-теориялық ұғымдар", "ақын-жазушылардың өмірі мен шығармашылығы", "поэзия талдауы", "прозалық шығармаларды талдау", "драмалық шығармалар", "кейіпкер бейнесі", "идея, тақырып, композиция", "тарихи-көркемдік маңызы", "автор позициясы", "әдеби шығарма бойынша салыстырмалы талдау"]
  },
  "Орыс тілі": {
    description: "Лексико-грамматические нормы русского языка.",
    topics: ["фонетика, лексика, грамматика", "морфология, синтаксис, пунктуация", "понимание текста, стилистика", "языковые нормы, употребление слов"]
  },
  "Орыс әдебиеті": {
    description: "Русская классическая и современная литература.",
    topics: ["авторы и произведения", "жанры, литературный анализ", "персонаж, идея, композиция", "историко-литературный контекст"]
  }
};
